...
public class EntityUI extends UI {
    ...
    @Override
	public void update() {
		character = HUD.get().getCursor().getHover();
	}
	...
	public void drawUI(Batch batch, float parentAlpha) {
		...
		if (character != null) {
			...
			batch.draw(ResourceManager.get(TextureMaps.UI)[0][3], getX() + HUD.FONT_SIZE, 12 + getY() + ((2) * HUD.FONT_SIZE), HEALTH_WIDTH * (
					character.getStats().getHealth() / Stats.MAX_HEALTH),
					HEALTH_HEIGHT);
			batch.draw(ResourceManager.get(TextureMaps.UI)[1][2], getX() + HUD.FONT_SIZE, 12 + getY() + ((1) * HUD.FONT_SIZE), HEALTH_WIDTH * (
				character.getStats().getMana() / Stats.MAX_MANA), HEALTH_HEIGHT);
